      ...
      ContainerDefinitions:
        - Cpu: 64
          ...
          DockerLabels:
            ...
            airflow.tasks.latest_only.depends.0: 'convert'
            airflow.tasks.repartition.args.0: '{{ ds }}'
            airflow.tasks.repartition.depends.0: 'latest_only'
          Environment:
            - Name: SAGEMAKER_TAG
              Value: !Ref SagemakerDockerTag
...
def main():
    parser = ArgumentParser(description='Run train job.')
    parser.add_argument('run_date', nargs=1, help='Date to run for.')
    args = parser.parse_args()
    convert(run_date=args.run_date[0])
```
